...
import frentix.HttpHeaders

/**
 * Created by srosse on 09.12.14.
 */
object QTI12TestPage94 extends HttpHeaders {
  ...
  def reloadFirstSectionToFinish: ChainBuilder = exec(session => {
      ...
      http("Reload first section")
        ...
        .check(status.is(200))
        .check(css("""div#o_qti_run_content_inner h3"""))
        .check(css("""#o_qti_run_title a.o_sel_submit_assess""","href")
          .find(0)
          .saveAs("finishTest")))
  ...
  def finish: ChainBuilder = exec(
      http("Finish test")
        ...
        .headers(headers)
        .check(status.is(200))
        .check(css("""#o_qti_run_title a.o_sel_close_assess""","href")
          .find(0)
          .saveAs("closeTest"))
